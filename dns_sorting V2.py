...
class DNS_file:
    ...
    def setup_LOOM_config(self, LOOM_path = None):
        ...
        if self.LOOM_file_exists: 
            ...
            logger.warning(f"The LOOM file {self.LOOM_file_path} does not exist.")
    ...
    def compare_DNS_entries(self):
        if not self.identical_to_LOOM:
            ...
            ip_in_DNS = [[line[3],line[0]] for line in line_not_in_DNS if line[3] in [line[3] for line in Tokenized_DNS_entries]]
            # name server that corresponds to the ip address in the LOOM file (the name server that has a different ip address in the DNS file from the LOOM file)
            name_server_of_ip_in_DNS = [line[0] for line in Tokenized_DNS_entries if line[3] in [line[0] for line in ip_in_DNS]]
            ...
            for line in line_not_in_DNS:
                if line[3] not in [line[0] for line in ip_in_DNS]:
                    print(f"\n\n------------------------------------------------------------------------------------")
                    print(f"The DNS entry that makes the ip address {line[3]} point to the server {line[0]} is present in the LOOM file {self.LOOM_file_path} but not in the local DNS file {self.name}.")
            ...
            for index, ip in enumerate(ip_in_DNS):
                print(f"\n\n", "-" * 80)
                print(f"In the LOOM file, the ip address {ip[0]} points to the server {ip[1]}. Meanwhile in the DNS file, it points to the server {name_server_of_ip_in_DNS[index]}.")
                print(f"In the LOOM file, the entry is")
                print(f"{' '.join(Tokenized_DNS_entries[[line[3] for line in Tokenized_DNS_entries].index(ip[0])])}\n") # I don't understand this line and I don't want to try to understand it. thx copilot
                print(f"In the DNS file, the entry is")
                print(f"{' '.join(Tokenized_LOOM_DNS_entries[[line[3] for line in Tokenized_LOOM_DNS_entries].index(ip[0])])}\n")
                
                for attempt in range(3):
                    answer = input(("would you like to change the entry in the DNS file to what is present in the LOOM file ? (y/N) : "))
                    if answer.lower() == "y":
                        logger.info(f"Changing the entry {' '.join(Tokenized_LOOM_DNS_entries[[line[3] for line in Tokenized_LOOM_DNS_entries].index(ip[0])])} to {' '.join(Tokenized_DNS_entries[[line[3] for line in Tokenized_DNS_entries].index(ip[0])])} ...") # again, I don't know what this is.
                        self.list_of_DNS_entries[[line[3] for line in Tokenized_DNS_entries].index(ip[0])] = ' '.join(Tokenized_LOOM_DNS_entries[[line[3] for line in Tokenized_LOOM_DNS_entries].index(ip[0])]) + '\n' #???????????????
                        break    
                    elif answer.lower() in ["n", ""]:
                        logger.info(f"The entry {' '.join(Tokenized_LOOM_DNS_entries[[line[3] for line in Tokenized_LOOM_DNS_entries].index(ip[0])])} will not be changed.")
                        break
                    else:
                        print(f"invalide answer, please try again.")
...
loom_path = "/home/ruddy/Documents/LOOM"
...
for file in DNS_files:
    ...
    logger.info(f"Compare the DNS entries of the file {file.name} with the LOOM file {file.LOOM_file_path} ...")
    file.compare_DNS_entries()
    logger.info(f"reconstructing the file {file.LOOM_file_path} ...")
